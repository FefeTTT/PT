...
$idUEA = isset($_GET['idUEA']) ? (int)$_GET['idUEA'] : 0;
if ($idTrimestre <= 0 || $idUEA <= 0) {
    http_response_code(400);
    echo json_encode(['ok'=>false,'error'=>'PARAM_INVALID']);
    exit;
}

try {
    $dao = new UEADAO($pdo);
    $data = $dao->detalleUEAPorTrimestre($idTrimestre, $idUEA);
