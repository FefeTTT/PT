...
function mostrarModalNuevoProfesor(){
    ...
    const selContrato = document.createElement('select'); selContrato.name = 'idProfesorTipo'; selContrato.className = 'form-select'; selContrato.required = true;
    const optCDefault = document.createElement('option'); optCDefault.value = ''; optCDefault.textContent = 'Seleccione tipo de contrato'; selContrato.appendChild(optCDefault);
    ...
    document.getElementById('btnGuardarProfesor').addEventListener('click', function(){
        ...
        // validar tipo de contrato (select obligatorio)
        const tipoContratoVal = (form.querySelector('select[name="idProfesorTipo"]') || {}).value || '';
        if (!tipoContratoVal) {
            if (window.swal) new swal('Error', 'Selecciona el Tipo de contrato', 'error'); else alert('Selecciona el Tipo de contrato');
            return;
        }
    ...
    fetch('controlador/ValidaProfesorExiste.php', { method: 'POST', body: checkPayload }).then(r => { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); }).then(check => {
            ...
            if (check.existsNombre){
                const msg = 'Ya existe un profesor con ese nombre' + (check.existingNombre && check.existingNombre.numeroEconomico ? (': ' + check.existingNombre.numeroEconomico) : '');
                if (window.swal) new swal('Error', msg, 'error'); else alert(msg);
                return;
            }
