...
$('input[type="file"]').on('change', function () {
  ...
  if ($(this).val != '') {
    if (ext == "xsl" || ext == "xlsx" || ext == "csv") {} else {
      $(this).val('');
...
function guardarTrim() {
  ...
  $.ajax({
    ...
    success: function success(resp) {
      console.log(resp);
      console.log("trimestre y fecha guardados"); // alert(resp);
    },
    ...
    complete: function complete(xhr, status) {// alert('Petición realizada');
    }
  });
}
...
function cargarCatalogo(tipo) {
  ...
  if (tipo == '1') {
    ...
    _formData.append('tipo', tipo);

    _formData.append('archivoexcel', files); // crear el llamado a ajax
    ...
    formData.append('archivoexcel', files); // crear el llamado a ajax
...
function cat_ejemplo(tipo) {
  if (tipo == 'uea') myWindow = window.open('img_catalogos.php?cat=uea', 'myWin', 'width=750,height=800');else if (tipo == 'docentes') myWindow = window.open('img_catalogos.php?cat=prof', 'myWin', 'width=910,height=750');else if (tipo == 'plan') myWindow = window.open('img_catalogos.php?cat=plan', 'myWin', 'width=1200,height=750');
}