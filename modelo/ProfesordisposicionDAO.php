...
class ProfesordisposicionDAO {
     ...
     */
    public function insertar(int $trimestreId, int $profesorId, int $estado = 1, ?string $notas = null){
        $sql = "INSERT INTO profesordisposicion (trimestre_idTrimestre, profesor_idProfesor, estado, notas) VALUES (?, ?, ?, ?)";
        $stmt = $this->conexion->prepare($sql);
        $ok = $stmt->execute([$trimestreId, $profesorId, $estado, $notas]);
        if (!$ok) return null;
    ...
    public function insertarMasivo(int $trimestreId, array $profesores, int $estado = 1, string $notasPrefix = ''): int {
        ...
        $count = 0;
        $stmt = $this->conexion->prepare("INSERT INTO profesordisposicion (trimestre_idTrimestre, profesor_idProfesor, estado, notas) VALUES (?, ?, ?, ?)");
        foreach ($profesores as $p) {
            $idProf = is_array($p) && isset($p['idProfesor']) ? (int)$p['idProfesor'] : (is_object($p) && isset($p->idProfesor) ? (int)$p->idProfesor : (int)$p);
            $notas = $notasPrefix;
            $ok = $stmt->execute([$trimestreId, $idProf, $estado, $notas]);
            if ($ok) $count++;
        }
        return $count;
    }
    ...
    public function buscarPorProfesorYTrimestre(int $profesorId, int $trimestreId): ?array {
        $st = $this->conexion->prepare("SELECT idProfesorDisposicion, trimestre_idTrimestre, profesor_idProfesor, estado, notas FROM profesordisposicion WHERE profesor_idProfesor = ? AND trimestre_idTrimestre = ? LIMIT 1");
        $st->execute([$profesorId, $trimestreId]);
        $r = $st->fetch(PDO::FETCH_ASSOC);
        return $r ?: null;
    }

    /** Actualizar estado por profesor y trimestre. Retorna true si se actualizó. */
    public function actualizarEstadoPorProfesorTrimestre(int $profesorId, int $trimestreId, int $estado): bool {
        // intentar actualizar si existe
        $st = $this->conexion->prepare("SELECT idProfesorDisposicion FROM profesordisposicion WHERE profesor_idProfesor = ? AND trimestre_idTrimestre = ? LIMIT 1");
        $st->execute([$profesorId, $trimestreId]);
        $r = $st->fetch(PDO::FETCH_ASSOC);
        ...
        $ins = $this->conexion->prepare("INSERT INTO profesordisposicion (trimestre_idTrimestre, profesor_idProfesor, estado, notas) VALUES (?, ?, ?, ?)");
        return (bool)$ins->execute([$trimestreId, $profesorId, $estado ? 1 : 0, 'actualizado manualmente']);
    }
    ...
    public function listarDisponiblesConEstatusPreferencias(int $idTrimestre): array {
        $sql = "SELECT 
                    ...
                    CASE WHEN EXISTS (
                        ...
                        WHERE pp.trimestre_idTrimestre = :idTr AND pp.profesor_idProfesor = p.idProfesor
                    ) THEN 1 ELSE 0 END AS filledPrefs
                FROM profesordisposicion pd
                INNER JOIN profesor p ON p.idProfesor = pd.profesor_idProfesor
