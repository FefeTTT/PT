...
import { KdeRedisEntry } from './KdeCacheManager';
...
export class KdeUnificadoCached {
    ...
    public async inicializar(
        ...
        console.log(`KdeUnificadoCached: Iniciando Bulk Load MGET desde Redis para ${redisKeysArr.length} keys KDE...`);
        ...
        for (let i = 0; i < redisKeysArr.length; i += chunkSize) {
            ...
            for (let j = 0; j < chunk.length; j++) {
                ...
                const match = /^KDE:v2:([^:]+):(.+)$/.exec(key) ?? /^KDE:([^:]+):(.+)$/.exec(key);
                if (!match) continue;
                const eco = match[1];
        ...
        if (kdeValues.length === 0) {
            throw new Error(
                'KdeUnificadoCached: No se encontraron predicciones KDE en Redis. ' +
