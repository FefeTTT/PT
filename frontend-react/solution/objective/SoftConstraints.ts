...
export interface ISoftConstraint {
    ...
    evaluar(grafo: GrafoBipartito, funcionZ: ZScoreFunction): number;
    evaluarAsync?(grafo: GrafoBipartito, funcionZ: ZScoreFunction, context?: EvaluationContext): Promise<number>;
    evaluarCandidatoAsync?(
                ...
                for (const bloque of bloques) {
                    ...
                    if (duracion > this._umbralHoras) {
                        ...
                        const sumaW = gruposUnicos.reduce(
                            (acc, gId) => acc + funcionZ(numEco, gId, grafo).score, 0
                        );
