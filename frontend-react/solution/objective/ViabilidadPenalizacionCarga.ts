...
/**
 * Respuesta del endpoint /get_horario_penalty_finetuned
 */
...
/**
 * Restriccion suave: Viabilidad de Carga por Penalizacion ML Finetuned.
 *
 * La ruta sincrona conserva el comportamiento historico para compatibilidad.
 * La ruta async usada por el elbow es estricta: espera ML real observado,
 * no usa baseline como sustituto y falla si no hay estadisticos reales.
 */
export class ViabilidadPenalizacionCarga implements ISoftConstraint {
     ...
     */
    evaluar(grafo: GrafoBipartito, _funcionZ: ZScoreFunction): number {
     ...
     */
    async evaluarAsync(grafo: GrafoBipartito, _funcionZ: ZScoreFunction, context?: EvaluationContext): Promise<number> {
        ...
        if (profesoresActivos === 0) return 0.0;
    ...
    private _evaluarBloqueoDominio(
        ...
        if (
            loadProbability !== undefined &&
            loadProbability < this._loadProbabilityThreshold
        ) {
    ...
    private async _actualizarPenaltyProfesor(numEco: number, grafo: GrafoBipartito, hash: string): Promise<void> {
        ...
        try {
            ...
            // silent fail: ruta legacy conserva el comportamiento anterior
        }
    }
}
