...
import { ProfesorDTO } from '../types/FrontendTypes';
...
export class EjectionChain {
    ...
    public mejorar(
        ...
        const optimizacion = this._optimizarZ(grafo, fsm, funcionZ);
    ...
    private _obtenerAreaDelGrupo(grafo: GrafoBipartito, idGrupo: number): number {
    ...
    private _buscarProfesoresDelAreaExcluyendo(
        ...
        return Array.from(grafo.profesores.values())
            .filter(p => p.idArea === idArea && p.numeroEconomico !== excluirNumEco);
    }

    private _buscarProfesoresDelArea(grafo: GrafoBipartito, idArea: number): ProfesorDTO[] {
        return Array.from(grafo.profesores.values())
            .filter(p => p.idArea === idArea);
    }
    ...
    private _optimizarZ(
        ...
        while (this._debeContinuarOptimizacion(estado)) {
            ...
            this._ejecutarIteracionSwap(grafo, fsm, funcionZ, movimiento, candidatos, estado);
        }
