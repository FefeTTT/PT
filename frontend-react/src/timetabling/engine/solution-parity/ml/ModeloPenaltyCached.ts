...
import type { BrowserScoreCacheReader, PenaltyPrediction } from './BrowserScoreCache';
        ...
        for (let i = 0; i < redisKeysArr.length; i += chunkSize) {
            ...
            for (let j = 0; j < chunk.length; j++) {
                ...
                try {
                    ...
                    const parsedKey = chunk[j].match(/^Penalty_([^_]+)_(.+)$/);
                    if (!parsedKey) continue;
                    const eco = parseInt(parsedKey[1], 10);
