import { fetchObservedPenalty } from './BrowserScoreCache';
import { canonicalizePenaltyPayload, penaltyObservedKey, serializePenaltyPayload } from './CacheKeys';
...
export class PenaltyObservedWatchdogError extends PenaltyObservedCacheError {
    constructor(message: string, key: string, context?: PenaltyObservedContext) {
...
export class PenaltyObservedCache {
    ...
    constructor(options: PenaltyObservedCacheOptions = {}) {
        ...
        this._fetcher = options.fetcher ?? ((payload, timeoutMs) => fetchObservedPenalty(baseUrl, payload, timeoutMs));
    }
    ...
    public static keyForPayload(payload: PenaltyObservedPayload): string {
        return penaltyObservedKey(payload);
    }

    public prime(payload: PenaltyObservedPayload, responseOrPenalty: number | PenaltyObservedResponse): void {
        const canonicalPayload = canonicalizePenaltyPayload(payload);
        const response = typeof responseOrPenalty === 'number'
            ? { total_penalty: responseOrPenalty }
            : this._normalizeResponse(responseOrPenalty);
        if (!response) return;
        this._setCache(PenaltyObservedCache.keyForPayload(canonicalPayload), response);
    }
    ...
    private _normalizeResponse(value: unknown): PenaltyObservedResponse | undefined {
        ...
        if (Array.isArray(candidate.loads)) {
            const values = candidate.loads.filter(v => typeof v === 'number' && Number.isFinite(v));
            if (values.length === candidate.loads.length) normalized.loads = values;
        }
        ...
        if (Array.isArray(candidate.load_probabilities)) {
            ...
            if (values.length === candidate.load_probabilities.length) normalized.load_probabilities = values;
        }
        ...
        if (Array.isArray(candidate.load_density_ratios)) {
            ...
            if (values.length === candidate.load_density_ratios.length) normalized.load_density_ratios = values;
        }
